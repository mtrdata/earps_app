...
with col3:
    min_psxg, max_psxg = float(df['PSxG +/- per 90'].min()), float(df['PSxG +/- per 90'].max())
    psxg = st.slider("PSxG +/- per 90", min_value=min_psxg, max_value=max_psxg, value=(min_psxg, max_psxg))
    
    min_crosses, max_crosses = float(df['Crosses Stopped % per 90'].min()), float(df['Crosses Stopped % per 90'].max())
